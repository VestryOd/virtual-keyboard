...
class VirtualKeyboard {
  constructor(alphabets) {
    this.data = alphabets;
    this.rootElement, this.keyBoard, this.textArea, this.header, this.pressed, this.keys, this.letters, this.mouseClicked, this.languageButton;
    this.codes = ["ControlLeft", "AltLeft"];
    this.capslocked = false;
    this.exceptions = ["Delete", "ArrowLeft", "ArrowUp", "ArrowRight", "ArrowDown", "Backspace"];
    this.defaultSet =
      [
        [
  ...
  generateKeyboard() {
    ...
    document.addEventListener("keydown", (event) => {
      if (!this.exceptions.includes(event.code)) {
  ...
  textAreaChangeInfo(value) {
    if (value === -1) {
      this.textArea.value = (this.textArea.value.length - 1 === "n" || this.textArea.value.length - 1 === "t") && this.textArea.value.length - 2 === "\\" ? this.textArea.value.slice(0, -2) : this.textArea.value.slice(0, -1);
    } else if (value === "&amp;"){
      this.textArea.value += "&";
    } else {
